...
/**
 * 扫描webpack config文件，获取webpack config对象，
 * 对webpack config与shell option进行整合
 * 对webpack config对象进行最加工处理后，返回webpack config对象
 * @param {*} yargs 检查参数对象
 * @param {*} argv shell options对象
...
module.exports = function(yargs, argv, convertOptions) {
  ...
  // configFiles用来存储webpack config文件路径
	var configFiles = [];
	...
	var i;
	if(argv.config) {
    // 如果shell option中指定了webpack config文件，则获取指定的config文件
...
// ];
	if(configFiles.length > 0) {
      ...
      // NOTE: 从config files中获取webpack config，并保存进options数组
  ...
  // 对webpack config对象进行加工
	...
	function processOptions(options) {
      ...
      // PrefetchPlugin 该插件内注册了make回调事件
