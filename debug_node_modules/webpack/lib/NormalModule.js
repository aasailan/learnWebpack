...
/**
 * @description js的源码文件会被封装成NormalModule
 * @class NormalModule
 * @extends {Module}
 */
class NormalModule extends Module {
    ...
    super();
    // 匹配过loader之后的module资源路径
    this.request = request;
    // 用户写在源码的module资源路径
		this.userRequest = userRequest;
    this.rawRequest = rawRequest;
    // 解析器对象，由moduleFactory创建好传入
		this.parser = parser;
		this.resource = resource;
    this.context = getContext(resource);
    // NOTE: 需要处理当前module的loader，在moduleFactory创建module实例时，准备好loaders对象后传入
    // loaders内部的元素如右边所示： { loader: '.../node_modules/babel-loader/lib/index.js' }
  ...
  // 创建一个loaderContext对象，该对象在loader运行时作为参数传入
    ...
    // LoaderPlugin LoaderOptionsPlugin LoaderTargetPlugin 插件会对loaderContext对象进行初始化
  ...
  // NOTE: 实际的构建模块函数，在这里调用loader对module进行处理
	doBuild(options, compilation, resolver, fs, callback) {
		this.cacheable = false;
		const loaderContext = this.createLoaderContext(resolver, options, compilation, fs);

    // NOTE: 借助loader，将non-js modules and thier dependencies  转换为js modules
		runLoaders({
      resource: this.resource,
      // loaders是已经解析成绝对路径的loader js文件
			loaders: this.loaders,
			context: loaderContext,
			readResource: fs.readFile.bind(fs)
		}, (err, result) => {
      // NOTE: 此时module已经经过loader转换，不管任何类型的module都已经转换为js module
			if(result) {
      ...
      // NOTE: 生成source对象，内部保存了module的源码文件信息
 ...
 /**
  * @description 实现父类方法，build当前module的入口方法
  * @param {*} options
  * @param {*} compilation
  * @param {*} resolver
  * @param {*} fs
 ...
 build(options, compilation, resolver, fs, callback) {
      ...
      // NOTE: 此时module经过loader处理，已经转换为js module
      ...
      // if so, exit here;
      // 处理对于一些指定不需要parser生成ast语法树的module
      // https://webpack-v3.jsx.app/configuration/module/#module-noparse
        ...
        // NOTE: 通过生成的ast，此时module.dependencies 已经解析完毕
			} catch(e) {
