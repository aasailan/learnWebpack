...
/**
 * @description 在Compiler的watch模式中，负责监视文件系统，并且在文件发生改变时
 * 重启Compiler的编译过程
 * @class Watching
 */
class Watching {
  ...
  // 开始一次新的编译过程
	_go() {
		...
		this.running = true;
    this.invalid = false;
    // NOTE: watch模式下，当文件改变时，重新触发一个编译过程
...
/**
 * @description compiler对象只在Webpack启动时构建一次，存在于webpack从启动到关闭的整个生命周期，
 * 代表了配置完备的Webpack环境。
 * Compiler 继承自Tapable类，
 * 借助继承的Tapable类，Compiler具备有被注册监听事件，以及发射事件触发hook的功能，
 * compiler会在构建的过程中触发生命周期事件，以此带动监听生命周期事件的plugin进行事件处理，
 * 进而形成webpack的插件机制。大多数面向用户的插件，都是首先在 Compiler 上注册的。
 * 值得注意的是，各种loader实际上也是在LoaderPlugin（内置插件）内被调用的，loader本质上是一个函数
 * 接受模块源文件作为输入，转换成浏览器适用的文件作为输出，而负责启动调用loader的正是LoaderPlugin
 * @class Compiler
 * @extends {Tapable}
 */
class Compiler extends Tapable {
 ...
 run(callback) {
  ...
  // 创建一个新的Compilation对象
    ...
    // NOTE: 准备好compilation对象后，触发compilation事件，让相关handlers可以在compilation对象上注册监听
  ...
  */
  compile(callback) {
    ...
    // 触发 before-compile 生命周期
      ...
      // NOTE: 触发make生命周期，在make生命周期内，开始加载module完成module的编译
			this.applyPluginsParallel("make", compilation, err => {
				if(err) return callback(err);

				compilation.finish();
        // NOTE: 关键方法：在seal方法内进行编译完的chunk封装，并最后输出为bundle
