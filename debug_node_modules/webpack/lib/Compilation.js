...
/**
 * @description Compilation主要有两个作用 1、Compilation的声明周期控制着一次具体的编译过程
 * 2、Compilation在一次编译过程中起着类似上下文的作用，其内部保存着编译过程中所有的module对象和chunk对象
 * @class Compilation
 * @extends {Tapable}
 */
class Compilation extends Tapable {
    ...
    // 生成entry chunk 文件的模板对象
    this.mainTemplate = new MainTemplate(this.outputOptions);
    // 生成non-entry chunk 文件的模板对象
    ...
    // 保存入口文件module对象
    this.entries = [];
    // NOTE: entry chunk 存储在这个数组
    // 对象结构为：{ name: 'chunkName', module: moduleObject }
		this.preparedChunks = [];
		this.entrypoints = {};
		this.chunks = [];
    this.namedChunks = {};
    // 缓存的module对象数组
    this.modules = [];
    // 以module.identifier()为key，缓存module对象
    this._modules = {};
    // 缓存source对象
    this.cache = null;
    ...
    // fileName 为key，source为value，存储即将输出的assets文件
		this.assets = {};
		this.errors = [];
		this.warnings = [];
    this.children = [];
    // 保存模块类型（key）和对应的模块构建工厂（value）
 ...
 /**
  * @description 将module对象添加到compilation中存储
  * @param {*} module
  * @param {*} cacheGroup
  * @returns {Module || Boolean} 当传入module存在于cache中，无需rebuild，返回module对象；否则返回boolean值
 ...
 /**
  * @description 传入一个module，对其进行build，并且触发build-module相关事件
  * @param {*} module 需要build的module
  * @param {*} optional
  * @param {*} origin
  * @param {*} dependencies
  * @param {*} thisCallback
 ...
 /**
  * @description 处理module对象的依赖module
  * @param {*} module
  * @param {*} callback
  * @memberof Compilation
  */
 ...
 /**
  * @description 根据模块的类型获取对应的模块工厂并创建模块对象  从entry module开始构建所有模块
  * @param {*} context
  * @param {*} dependency
  * @param {function} onModule 当module 对象创建准备完毕回调
 ...
 _addModuleChain(context, dependency, onModule, callback) {
    ...
    // NOTE: 根据模块的类型获取对应的模块工厂并创建模块
      ...
      // NOTE: 调用moduleFactory创建modele对象
        ...
        // 将module对象缓存(add)到compilation对象
        ...
        // NOTE: 此时module对象已经创建准备好，但还没有进行构建
        onModule(module);
        ...
        // NOTE: 构建模块
          ...
          this.semaphore.release();
          // NOTE: 处理entry module的依赖module，此时entry module 已经构建完毕
 ...
 addEntry(context, entry, name, callback) {
    ...
    // preparedChunks 内存储者entry chunk; 处理entry chunk
    ...
    // optimize-chunks commonsChunkPlugins监听了 optimize-chunks
  ...
  // 生成一个chunk对象
	addChunk(name, module, loc) {
        ...
        // 如果在namedChunks中有当前chunk的缓存，则直接取出
  ...
  // 从entry module开始，递归的为所有module对象添加index 和 index2属性
	assignIndex(module) {
		const _this = this;
  ...
  // 从entry module开始，递归的为所有module设置depth属性
  // depth属性描述一个module对象的深度（module到达entry module的最短路径）
  // entry module的depth为0，entry依赖的module的depth为1，以此类推
	assignDepth(module) {
  ...
  // 该方法根据module图创建一个chunk图，从entry chunk开始，递归的处理所有chunk和module之间的联系
    ...
    // 保存新建的chunk对象
		...
		// PART ONE
    // 为每一个block创建一个chunk对象 Map<block, chunk>
    ...
    // 遍历所有block
		const iteratorBlock = b => {
          ...
          // NOTE: 为block创建一个新的chunk对象，并且保存到compilation对象
		...
		while(queue.length) {
			...
			if(block.dependencies) {
        // 对dependencies中的每一个dependency对象都调用iteratorDependency
				iterationOfArrayCallback(block.dependencies, iteratorDependency);
			}
