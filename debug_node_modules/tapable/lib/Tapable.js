...
Tapable.prototype.logPluginApply = function (pluginNames) {
  if (logConfig.logPluginApplyEnable) {
    ...
    console.log('插件注册: ' + className + ' registerPlugin: ' + pluginNames.join(', '));
  }
}
...
Tapable.prototype.logApply = function (funName, eventName) {
  if (logConfig.logApplyEnable) {
    var className = this.constructor.name;
    var handlers = this._plugins[eventName] || [];
    console.log('事件触发: ' + className + ' ' + funName + ': ' + eventName + ' handlerNum: ' + handlers.length);
  }
}
...
Compiler applyPluginsBailResult: entry-option // NOTE: option初始化
Compiler plugin: compilation
...
Compiler applyPluginsAsyncSeries: run // NOTE: 关键事件节点 Before reading records
Compiler applyPluginsAsyncSeries: before-compile
Compiler applyPlugins: this-compilation
Compilation plugin: child-compiler
Compiler applyPlugins: compilation // NOTE: 关键事件节点 Compilation creation completed，留意到创建完Compilation对象后马上注册了许多事件
...
Compiler applyPluginsParallel: make // NOTE: 关键事件节点 分析入口文件，创建文件对象，留意到make开始构建流程后，整个模块的编译过程都是Compilation的生命周期在控制
Compilation applyPlugins: normal-module-loader // NOTE: 关键事件节点 this is where all the modules are loaded one by one, no dependencies are created yet
...
Compiler applyPluginsAsyncSeries: after-compile // NOTE: 完成所有模块构建，结束编译过程
Compiler applyPluginsBailResult: should-emit
Compiler applyPluginsAsyncSeries: emit // NOTE: Before emitting assets to output dir，插件修改assets的最后机会
Compiler applyPluginsAsyncSeries1: after-emit // NOTE: assets输出完成
